...
(function(){
  const u=["utm_source","utm_medium","utm_campaign","utm_term","utm_content"],
        d={utm_source:"website",utm_medium:"unknown_page",utm_campaign:"general",utm_content:"cta_formulaire",utm_term:""},
        c=window.PAGE_UTM_CONFIG||{},
        p=window.location.pathname,
        g=(p.startsWith("/campus/")&&p!=="/campus/")
          ? {...d,utm_source:"website",utm_medium:"organic_search",utm_campaign:`data_analyst_distance_${p.split("/").pop()}`,utm_content:"cta_formulaire"}
          : {...d,...(c[p]||{})};
  ...
  const l=()=>{
    const e=JSON.parse(sessionStorage.getItem("initialUtms")||"null"),
          r=s(),
          t=g;
  ...
  const f=t=>{
    if(!t) return;
    const e=l();
    console.log("[UTM] Injection dans formulaire:",e);
    for(const[k,v]of Object.entries(e)){
      let i=t.querySelector(`input[name="${k}"]`);
      if(!i){
  ...
  const b=e=>{
    ...
    if(t) return t;
    for(const i of e.querySelectorAll("iframe")){
      try{
    ...
    return e.querySelector(".hs-form");
  };

  // ⚙️ Partie corrigée — ne modifie plus jamais l’iframe meetings
  const m=()=>{
    const i=document.querySelector(".meetings-iframe-container iframe");
    if(!i) return false;
    // On vérifie seulement l’existence, sans rien changer
    console.log("[UTM] Iframe meetings détectée, aucune modification appliquée.");
    return true;
  };

  const a=()=>{
    // Si une iframe meetings est détectée, on ne modifie rien et on continue
    m();
    const e=document.querySelector('[data-hubspot-form-container="true"]'),
          t=b(e);
    if(t){
      f(t);
      if(!t._utmListenerAdded){
        t.addEventListener("submit",()=>f(t));
        t._utmListenerAdded=true;
      }
    }
  };
  ...
  const init=()=>{
    a();
    [1e3,2e3,3e3,5e3].forEach(t=>setTimeout(a,t));
    const o=new MutationObserver(()=>a());
    o.observe(document.body,{childList:true,subtree:true});
    setTimeout(()=>o.disconnect(),3e4);
  };
